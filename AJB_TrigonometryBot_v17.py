...
def handleMentions(api,max_id,words): # tweetbot
			...
			if handleMention == True and replyToName != MYNAME:
				...
				if len(status["entities"]) > 0 and "media" in status["entities"].keys():
					...
					if len(userImgs) > 0:
						...
						seedword = msg.split()
						tweet_text = getTweetTextSentence(words,"I made this for you, "+replyToName+", because you asked nicely.\n") # getTweetText(tweetbot,"I made this for you, "+replyToName+", because you asked nicely.\n",seedword) # Conversation management goes here
						while len(tweet_text) > TWITTERLIMIT: # Twitter limit
							tweet_text = getTweetTextSentence(words,"I made this for you, "+replyToName+", because you asked nicely.\n") # getTweetText(tweetbot,replyToName,seedword)
						print tweet_text
						...
						tweet_text = getTweetTextSentence(words,"I made this for you, "+replyToName+", because you asked nicely.\n") # getTweetText(tweetbot,"I made this for you, "+replyToName+", because you asked nicely.\n",seedword) # Conversation management goes here
						while len(tweet_text) > TWITTERLIMIT: # Twitter limit
							tweet_text = getTweetTextSentence(words,replyToName) # getTweetText(tweetbot,replyToName,seedword)
						img = beCreative(width,height,replyToName) # Choose what type of image to create
						print tweet_text
						postToTwitter_Image(api,img,tweet_text,id)
